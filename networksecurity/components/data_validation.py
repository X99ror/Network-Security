...
class DataValidation:
    def __init__(self,data_ingestion_artifact:DataIngestionArtifact,data_validation_config:DataValidationConfig):
        try:
            self.data_ingestion=data_ingestion_artifact
            self.data_validation_config=data_validation_config
            self._schema_confnig=read_yaml_file(SCHEMA_FILE_PATH)
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            number_of_columns=len(self._schema_confnig)
            print(f"Required number of columns{number_of_columns}")
            print(f"dataframe has columns{dataframe.columns}")
    ...
    def detect_data_drift(self,base_df,current_df,threshold)->bool:
        try:
            status=True
            report={}
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
            try:
                train_file_path=self.data_ingestion.trained_file_path
                ...
                status=self.detect_data_drift(base_df=train_dataFrame,current_df=test_dataFrame,threshold=0.5)
                ...
                data_validation_artifact= DataValidationArtifact(
                    ...
                    valid_train_file_path=self.data_ingestion.trained_file_path,
                    valid_test_file_path=self.data_ingestion.test_file_path,
